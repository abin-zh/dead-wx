...
<script setup>
	...
	const login = () => {
		myForm.value.validate(() => {
			postRequest("api/mobile/elogin",loginModel).then(res=>{
				if(res.success){
					userStore.fillUser(res.data.userinfo)
					uni.switchTab({
						url:'/pages/my/my'
					})
